...
export class InvoiceService {
    ...
    public async getPaginated(page: number, pageSize: number): Promise<{ data: InvoiceVwModel[], total: number }> {
        return {
            data: [
                {
                    Id: 'guid-123',
    ...
    // Create new invoice
    public async createInvoice(invoice: Omit<Invoice, 'Id' | 'CreatedDate'>): Promise<Invoice> {
        Toast.fire({
            icon: "success",
    ...
    public async editInvoice(id: string, invoice: Partial<Invoice>): Promise<Invoice> {
        Toast.fire({
            icon: "success",
